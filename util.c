...
long readFile(char *name, void **buffer) {
    ...
    if (strcmp("-", name) == 0) {
        ...
        if (!file)
        {
            php_error_docref(NULL, E_WARNING, "Unable to open file: %s", name);
            return 0;
        }
    }
    ...
    while ((bytesRead = fread(chunk, 1, sizeof chunk, file)) > 0) {
        ...
        if (reallocated) {
            *buffer = reallocated;
            ...
            php_error_docref(NULL, E_WARNING, "Only able to read %zu bytes!0", fileLen);
            free(*buffer);
            fclose(file);
            return 0;
        }
...
unsigned long decodePpm(unsigned char *buf, unsigned long bufSize, unsigned char **image, int *width, int *height) {
    ...
    if (!checkPpmMagic(buf, bufSize)) {
        php_error_docref(NULL, E_WARNING, "Not a valid PPM format image!");
        return 0;
    }
    ...
    if (depth != 255) {
        php_error_docref(NULL, E_WARNING, "Unsupported bit depth: %d", depth);
        return 0;
    }
    ...
    if (pos + imageDataSize != bufSize) {
        php_error_docref(NULL, E_WARNING, "Incorrect image size: %lu vs. %lu", bufSize, pos + imageDataSize);
        return 0;
    }
